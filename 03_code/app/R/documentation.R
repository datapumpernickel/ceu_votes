...
minority_table_docu <- list(
  tags$p(
   "Minority Voting refers to a scenario where a measure or policy proposal is adopted despite one or more member states not voting in favour. The Council Monitor differentiates between states voting against or abstaining. Not participating refers to cases where a member state does not participate in a vote due to an opt-out from a policy area, e.g. Ireland on Schengen."
  ),
  tags$p(
    tags$b("Policy Areas"),br(),
...
doku_nav <- function(id, label = "doku") {
  ...
  tagList(fluidPage(
                    fluidRow(
                      column(9,
                             card(includeHTML(rmarkdown::render("www/documentation/documentation.Rmd",
                                                             params = list(nrows=nrow(df_all_votes_table),
                                                                           date = date))),
