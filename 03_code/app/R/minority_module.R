        ...
        div(
          add_prompt(
            selectizeInput(
              ...
              selected = "All",
              choices = c("All", sort(unique(df_consensus$policy_area_label[df_consensus$policy_area_label != "Missing"])), "Missing"),
              multiple = TRUE,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      selected_policy_areas <- reactive({
        req(input$ceu_config_type)
        selected_areas <- input$ceu_config_type
        selected_areas
      })
      ...
      output$table <- renderDT(server=FALSE,
                               {
                                 ...
                                 table_data <- filtered_data() |> select(-country) |> 
                                   relocate(country_full, .before = "policy_area_label") |> 
                                   group_by(country_full, policy_area_label) |> 
                                   summarise(infavor = sum(infavor),
                                             abstained = sum(abstained),
                                             against = sum(against),
                                             `not participating` = sum(`not participating`),.groups = "drop") 
      ...
      output$plot <- renderPlotly({
        ...
        # Adjust bar width based on the number of unique x values
        # You might need to tweak the values based on your specific needs
        if(!is.null(input$country_type)){
          plot_width <- width
          
          # Calculate plot width based on the number of countries selected
          if (length(input$country_type) < 10) {
            plot_width <- width * max(c((length(input$country_type)/10),0.3))
          }
          
          # Adjust plot width if two policy areas are selected
          if (length(input$ceu_config_type) > 1) {
            plot_width <- plot_width * 2
          }
          
          # Subtract 0.05 of the total width from the final plot width to ensure it fits into the box
          
          # Ensure plot_width does not exceed the canvas width and is not less than 0
          plot_width <- min(max(plot_width, 0), width)   
          plot_width <- plot_width - 0.05 * width
          ...
          plot_list <- unique_policy_areas %>%
            ...
            group_map(~ {
              ...
              plot_data() %>%
                ...
                layout(yaxis = list(title = 'Percentage', titlefont = list(size = 16), tickfont = list(size = 14), tickformat = ".1%"),
                       xaxis = list(tickangle = -45, tickfont = list(size = 14), autorange = "reversed"),
                       barmode = 'stack',
                       margin = list(t = 60),
